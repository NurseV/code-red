...
import { PlusIcon, ListChecksIcon, XIcon, EditIcon } from '../components/icons/Icons';
...
const ChecklistTemplates: React.FC = () => {
    ...
    return (
        <>
            <Card 
                title="Apparatus Checklist Templates"
                actions={<Button onClick={handleOpenCreateModal} icon={<PlusIcon className="h-4 w-4 mr-2" />}>New Template</Button>}
            >
                {isLoading ? <div className="text-center p-8 text-dark-text-secondary">Loading templates...</div> : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {templates.map(template => (
                            <div key={template.id} className="bg-dark-card border border-dark-border rounded-lg p-4 flex flex-col">
